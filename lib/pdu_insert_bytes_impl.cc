...
void pdu_insert_bytes_impl::forecast(int noutput_items, gr_vector_int& ninput_items_required)
{
}
...
void pdu_insert_bytes_impl::msg_handler(pmt::pmt_t pmt_msg)
{
    ...
    extended_msg.insert(std::next(extended_msg.begin(), d_pos), d_bytes.begin(), d_bytes.end());

    message_port_pub(
        pmt::mp("out"),
        pmt::cons(pmt::car(pmt_msg), pmt::init_u8vector(extended_msg.size(), extended_msg)));
}
